...
public class DetectionCore {
	...
	//mavi kapak contur u buradan geliyor
	public List<MatOfPoint> getContoursBlueMask(){
	    ...
	    /*
	    mTemp=new Mat();
	    Core.inRange(lHSV.get(0), new Scalar(0), new Scalar(255), mTemp);
	    lHSV.set(0, mTemp);
	...
	//////daire arama elimizdeki hatlarda  ////////////////////////////////////////////////////////
	private void FindCircle(List<MatOfPoint> contours, int index){
    	...
    	do{
		    ...
		    if(Imgproc.contourArea(contour) > 500){
		    	...
		    	RotatedRect bound = Imgproc.minAreaRect(specialPointMtx);
                ...
                if(bound.size.height<bound.size.width)
                	uzunlukOran =(bound.size.width ) / (bound.size.height );
                else 
                	uzunlukOran =(bound.size.height ) / (bound.size.width );
...
//eksenlerin oran›ndan cismin dairesel yada elips gibi oldugu bilgisini c›kar›yor.     	  //////////////////////////////////////////  
		    	double longAxis;
                double shortAxis;
...
///////////////////////////////////////////////////////////////////////////////////////
        //        Log.i("-------yukseklik ----  ", " "+bound.size.height);
        //        Log.i("-------en ----  ", " "+bound.size.width);
                Log.i("-------oran ----  ", " "+bound.size.height/bound.size.width);
...
package net.ercsoft.nesnetanima;
...
public class DetectionCore {
	...
	public List<MatOfPoint> getContoursBlueMask(){
		...
		mTemp=new Mat();
		Core.inRange(lHSV.get(0), new Scalar(90), new Scalar(130), mTemp);  //90-130
		lHSV.set(0, mTemp);
	...
	private void FindCircle(List<MatOfPoint> contours, int index){
		...
		do{
			...
			if(Imgproc.contourArea(contour) > 500){
				...
				//this could stop the searching when is elipse too oval 
				double eksenlerOrani=longAxis / shortAxis;
				if ((longAxis / shortAxis) < 2.0){
					Log.i("***", "elips bulundu");
					elipsmi=1;
					signList.add(roi);
				...
				String parametreler="k_oran:"+bound.size.height/bound.size.width+"**k_eksenlerOrani:"+eksenlerOrani;
				Log.i("", parametreler);
			}
			//there are no next contour
		}while(i != -1);
